...
export async function POST(req: Request) {
  try {
    ...
    const manageable = await canManageChild(userId, {
      userId: child.userId,
      familyId: child.familyId,
    });
    if (!manageable) {
      return NextResponse.json({ error: '没有权限推送该孩子的学习简报' }, { status: 403 });
    }
    ...
    const plan = await prisma.weeklyPlan.findFirst({
      where: { childId: body.childId, weekId },
    });
    if (!plan) {
    ...
    console.error('[dingtalk/push] error:', err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : '推送失败，请稍后重试' },
      { status: 500 }
    );
  }
}
