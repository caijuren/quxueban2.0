...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { registrationType, householdType, continuousYears, hasPropertyInDistrict, socialInsuranceYears } = parsed.data;
    ...
    if (householdType === '非本市户籍' && continuousYears < 3) {
      ...
      eligibility.reason = '非本市户籍学生通常需要连续三年在本市初中就读，并满足居住证积分或房产等要求。';
    ...
    if (householdType === '非本市户籍' && continuousYears >= 3) {
      ...
      if (hasResidence) {
        ...
        eligibility.reason = '满足连续三年本市就读，且具备居住证积分/房产/社保等条件，可申请名额到校。';
        eligibility.suggestions = ['向学校确认名额到校志愿填报流程。', '关注当年中考政策微调。'];
      } else {
        eligibility.level = 'unlikely';
        eligibility.reason = '连续三年就读但缺少居住证积分或房产等辅助条件，名额到校资格存在不确定性。';
    ...
    if (householdType === '本市户籍') {
      if (continuousYears >= 3) {
        ...
        eligibility.suggestions = ['关注一模、二模成绩对名额到校录取的影响。', '合理填报志愿，区分名额分配到区与到校。'];
      } else if (continuousYears >= 1) {
        eligibility.eligible = true;
        eligibility.level = 'possible';
        eligibility.reason = '本市户籍但在本市初中就读未满三年，可能按学籍所在区政策认定，具体以当年政策为准。';
        eligibility.suggestions = ['向学校教务处确认连续就读年限认定标准。', '关注中考报名资格审核通知。'];
      } else {
        eligibility.level = 'unlikely';
