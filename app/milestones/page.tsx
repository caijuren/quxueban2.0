import { Metadata } from 'next';
import PageLayout from '@/components/marketing/PageLayout';
import TimelineVisual from '@/components/marketing/TimelineVisual';
import { Bell, Layers, CheckCircle2 } from 'lucide-react';

export const metadata: Metadata = {
  title: '里程碑 - 趣学伴',
  description: '把升学目标拆解成可执行的时间节点，按年级、学期、月度追踪关键任务',
};

export default function MilestonesMarketingPage() {
  return (
    <PageLayout ctaText="立即建立里程碑">
      {/* Hero */}
      <section className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8 overflow-hidden min-h-[90vh] flex items-center">
        <div className="max-w-7xl mx-auto w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div>
              <span className="text-xs font-mono text-primary uppercase tracking-widest mb-4 block">
                Milestones
              </span>
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black font-display leading-[0.95] mb-6">
                把大目标
                <br />
                <span className="text-slate-500">拆成可执行节点</span>
              </h1>
              <p className="text-lg text-slate-400 max-w-lg mb-10 leading-relaxed">
                升学不是一蹴而就。趣学伴将长远目标拆分为年级目标、学期目标、月度任务和周计划，让每一步都有明确的截止时间。
              </p>
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2 px-4 py-2 rounded-lg border border-white/5 bg-surface/30 text-sm text-slate-300">
                  <Layers className="w-4 h-4 text-primary" />
                  层级拆解
                </div>
                <div className="flex items-center gap-2 px-4 py-2 rounded-lg border border-white/5 bg-surface/30 text-sm text-slate-300">
                  <Bell className="w-4 h-4 text-secondary" />
                  节点提醒
                </div>
                <div className="flex items-center gap-2 px-4 py-2 rounded-lg border border-white/5 bg-surface/30 text-sm text-slate-300">
                  <CheckCircle2 className="w-4 h-4 text-accent" />
                  进度可视化
                </div>
              </div>
            </div>
            <TimelineVisual />
          </div>
        </div>
      </section>

      {/* Grade breakdown */}
      <section className="py-24 px-4 sm:px-6 lg:px-8 border-t border-white/5">
        <div className="max-w-6xl mx-auto">
          <div className="space-y-6">
            {[
              {
                grade: '年级目标',
                example: '五年级完成三公报名并取得录取',
                detail: '明确这一年的终极目标和验收标准',
              },
              {
                grade: '学期目标',
                example: '上学期完成 AMC8 首考，下学期冲刺高分',
                detail: '把年级目标拆到上下学期，便于跟踪',
              },
              {
                grade: '月度任务',
                example: '9 月完成小托福报名，10 月参加模考',
                detail: '每月有具体动作，不遗漏关键准备',
              },
              {
                grade: '周计划',
                example: '本周完成 3 次奥数练习 + 1 次英语阅读',
                detail: '落实到每周可执行的学习安排',
              },
            ].map((item, index) => (
              <div
                key={item.grade}
                className="group flex flex-col md:flex-row md:items-center gap-4 p-6 rounded-2xl border border-white/5 bg-surface/30 hover:border-primary/20 transition-colors duration-300"
              >
                <div className="md:w-48 shrink-0">
                  <span className="text-xs font-mono text-primary">LEVEL 0{index + 1}</span>
                  <h3 className="text-xl font-bold font-display mt-1">{item.grade}</h3>
                </div>
                <div className="flex-1">
                  <p className="text-white font-medium mb-1">{item.example}</p>
                  <p className="text-sm text-slate-400">{item.detail}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>
    </PageLayout>
  );
}
