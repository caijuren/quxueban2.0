...
const targetPresets = [
  { icon: School, color: 'from-primary to-primary-glow', shadow: 'hover:shadow-glow-primary' },
  { icon: GraduationCap, color: 'from-secondary to-purple-400', shadow: 'hover:shadow-glow-secondary' },
  { icon: Home, color: 'from-accent to-cyan-400', shadow: 'hover:shadow-panel' },
  { icon: Globe, color: 'from-secondary to-secondary-glow', shadow: 'hover:shadow-glow-secondary' },
  { icon: Layers, color: 'from-indigo-500 to-purple-500', shadow: 'hover:shadow-glow-secondary' },
  { icon: Scale, color: 'from-accent to-accent-glow', shadow: 'hover:shadow-panel' },
];
      ...
      : [
          {
            ...
            tag: '请完善',
            icon: School,
            color: 'from-border-strong to-border-default' as const,
            shadow: 'hover:shadow-panel' as const,
          },
        ];
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-full max-w-lg max-h-[85vh] rounded-3xl flex flex-col overflow-hidden bg-surface border border-primary/30 shadow-dropdown"
        >
          <div className="relative z-10 p-6 pb-4 border-b border-border-subtle flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-text-tertiary">添加一条自定义升学路线</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="relative z-10 flex-1 overflow-y-auto p-6">
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-surface-light border border-border-default text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary transition-all"
                  required
                />
              </div>

              <div>
                <label className="block text-sm text-text-secondary mb-1">方案类型</label>
                  ...
                  <button
                    ...
                    className={`flex-1 px-4 py-2 rounded-xl text-sm font-medium border transition-all ${
                      type === 'primary'
                        ...
                        : 'bg-surface-light border-border-subtle text-text-secondary hover:bg-surface-highlight'
                    }`}
                  >
                    主路线
                  </button>
              ...
              <div>
                <label className="block text-sm text-text-secondary mb-1">方案说明</label>
                <textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="简要说明该路线的目标与策略"
                  rows={3}
                  className="w-full px-4 py-2.5 rounded-xl bg-surface-light border border-border-default text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-primary transition-all resize-none"
                />
              </div>

              <div>
                <label className="block text-sm text-text-secondary mb-1">关键要求（用逗号或换行分隔）</label>
                <textarea
                  value={requirements}
              ...
              <div>
                <label className="block text-sm text-text-secondary mb-1">路线匹配度：{probability}%</label>
                <input
                  type="range"
                  min={0}
                  max={100}
            ...
            <div className="mt-6 pt-4 border-t border-border-subtle flex items-center justify-end gap-3">
              ...
              <button
                type="submit"
                className="px-4 py-2 rounded-lg bg-gradient-to-r from-primary to-secondary text-text-primary text-sm font-medium hover:shadow-glow-primary transition-all"
              >
                创建方案
              </button>
            </div>
