        ...
        >
          <div className="relative z-10 p-6 pb-4 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">添加一条自定义升学路线</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="relative z-10 flex-1 overflow-y-auto p-6">
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-slate-400 mb-1">方案名称</label>
                <input
                  type="text"
                  value={name}
              ...
              <div>
                <label className="block text-sm text-slate-400 mb-1">方案类型</label>
              ...
              <div>
                <label className="block text-sm text-slate-400 mb-1">方案说明</label>
                <textarea
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="简要说明该路线的目标与策略"
                  rows={3}
                  className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-primary transition-all resize-none"
                />
              </div>

              <div>
                <label className="block text-sm text-slate-400 mb-1">关键要求（用逗号或换行分隔）</label>
                <textarea
                  value={requirements}
              ...
              <div>
                <label className="block text-sm text-slate-400 mb-1">路线匹配度：{probability}%</label>
                <input
                  type="range"
                  min={0}
                  max={100}
