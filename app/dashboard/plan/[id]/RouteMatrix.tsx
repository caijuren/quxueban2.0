...
const priorityConfig = {
  must: { label: '必须', color: 'text-primary', bg: 'bg-primary/10', border: 'border-primary/30' },
  should: { label: '建议', color: 'text-warning', bg: 'bg-warning/10', border: 'border-warning/30' },
  optional: { label: '可选', color: 'text-text-secondary', bg: 'bg-surface-highlight', border: 'border-border-subtle' },
};
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-slate-500/10 text-text-secondary border border-slate-500/30">
      ...
      <div className="overflow-x-auto -mx-2 px-2 pb-2">
        <div className="min-w-[780px]">
          ...
          <div className="grid grid-cols-[150px_repeat(5,1fr)] gap-2 mb-2">
            <div className="px-3 py-2 text-sm font-medium text-text-secondary flex items-end">
              分类 / 年级
            </div>
            {grades.map((grade) => {
              ...
              return (
                <div
                  ...
                  className={`px-2 py-2 rounded-lg text-center flex flex-col items-center justify-center min-h-[48px] ${
                    isCurrent
                      ? 'bg-gradient-to-r from-primary to-secondary text-text-primary shadow-glow-primary'
                      : 'bg-surface-light text-text-secondary border border-border-subtle'
                  }`}
                >
                  <span className="text-xs font-bold">{grade}</span>
                  <span className={`text-[10px] mt-0.5 ${isCurrent ? 'text-text-primary/80' : 'text-text-secondary'}`}>
                    {timeRange}
                  </span>
                </div>
              );
          ...
          <div className="space-y-2">
            {rows.map((row, rowIndex) => {
              ...
              return (
                ...
                >
                  {/* Row header */}
                  <div className="px-3 py-3 rounded-xl bg-surface-light border border-border-subtle flex items-center gap-3">
                    <div
                    ...
                    <span className="text-sm font-bold text-text-primary">{row.category}</span>
                  </div>
                  ...
                  {grades.map((grade) => {
                    ...
                    if (!cell) {
                      return (
                        <div
                          key={grade}
                          className="rounded-xl bg-surface-light border border-border-subtle min-h-[100px]"
                        />
                      );
                    }
                    ...
                    return (
                      <motion.div
                        ...
                        className={`group relative rounded-xl p-3 border min-h-[100px] flex flex-col justify-between transition-all cursor-default ${
                          isCurrent
                            ? `${priority.bg} ${priority.border} shadow-glow-primary`
                            : 'bg-surface-light border-border-subtle hover:border-border-default hover:bg-surface-highlight'
                        }`}
                      >
                        <div>
