...
const priorityConfig = {
  ...
  optional: { label: '可选', color: 'text-slate-600', bg: 'bg-slate-500/10', border: 'border-slate-500/30' },
};
      ...
      <div className="mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-2">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-slate-500/10 text-slate-600 border border-slate-500/30">
      ...
      <div className="overflow-x-auto -mx-2 px-2 pb-2">
        <div className="min-w-[780px]">
          ...
          <div className="grid grid-cols-[150px_repeat(5,1fr)] gap-2 mb-2">
            <div className="px-3 py-2 text-sm font-medium text-slate-600 flex items-end">
              分类 / 年级
            </div>
            {grades.map((grade) => {
              ...
              return (
                <div
                  ...
                  className={`px-2 py-2 rounded-lg text-center flex flex-col items-center justify-center min-h-[48px] ${
                    isCurrent
                      ? 'bg-gradient-to-r from-primary to-secondary text-text-primary shadow-[0_0_20px_rgba(244,63,94,0.3)]'
                      : 'bg-black/5 text-slate-700 border border-white/5'
                  }`}
                >
                  <span className="text-xs font-bold">{grade}</span>
                  <span className={`text-[10px] mt-0.5 ${isCurrent ? 'text-text-primary/80' : 'text-slate-600'}`}>
                    {timeRange}
                  </span>
                </div>
              );
          ...
          <div className="space-y-2">
            {rows.map((row, rowIndex) => {
              ...
              return (
                ...
                >
                  {/* Row header */}
                  <div className="px-3 py-3 rounded-xl bg-black/[0.03] border border-white/5 flex items-center gap-3">
                    <div
                  ...
                  {grades.map((grade) => {
                    ...
                    if (!cell) {
                      return (
                        <div
                          key={grade}
                          className="rounded-xl bg-black/[0.02] border border-white/5 min-h-[100px]"
                        />
                      );
                    }
                      ...
                      <motion.div
                        ...
                        className={`group relative rounded-xl p-3 border min-h-[100px] flex flex-col justify-between transition-all cursor-default ${
                          isCurrent
                            ...
                            : 'bg-black/[0.03] border-white/5 hover:border-white/20 hover:bg-black/[0.05]'
                        }`}
                      >
                        <div>
