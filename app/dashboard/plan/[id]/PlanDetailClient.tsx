  ...
  if (!plan) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-text-secondary mb-6">该路线方案不存在或已被删除</p>
        <Link
          href="/dashboard/plan"
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-text-primary font-semibold hover:shadow-glow-lg transition-all"
        >
          返回路线方案
        </Link>
      </div>
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            <div className="flex flex-wrap gap-2">
              {plan.requirements.map((req) => (
                <span
                  key={req}
                  className="px-3 py-1.5 rounded-lg bg-surface-light text-sm text-text-secondary border border-border-subtle"
                >
                  {req}
                </span>
              ))}
          ...
          <div className="flex flex-col justify-center items-center lg:items-end">
            <p className="text-sm text-text-secondary mb-1">路线匹配度</p>
            <p
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-secondary">{school.tag}</p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {planResources.map((resource, index) => (
            <motion.div
              ...
              className="rounded-xl bg-surface-light p-4 border border-border-subtle hover:border-primary/30 hover:bg-surface-highlight transition-all cursor-pointer group"
            >
              <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center mb-3 group-hover:bg-primary/20 transition-colors">
                <resource.icon className="w-5 h-5 text-primary" />
              </div>
              <h4 className="font-medium text-text-primary mb-1">{resource.title}</h4>
              <p className="text-xs text-text-secondary">{resource.desc}</p>
            </motion.div>
          ))}
        </div>
      </motion.div>
      ...
      >
        ...
        <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-text-primary font-semibold hover:shadow-glow-lg transition-all">
          制定提升计划
