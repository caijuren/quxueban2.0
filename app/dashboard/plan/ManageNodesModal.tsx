        ...
        >
          <div className="relative z-10 p-6 pb-4 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">编辑各条路线的关键里程碑</p>
              </div>
            </div>
            <button
              onClick={onClose}
            ...
            {draftPlans.map((plan) => (
              <div key={plan.id}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="font-bold font-display text-slate-800">{plan.name}</h4>
                </div>
                  ...
                  {plan.milestones.map((milestone, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="时间"
                        className="w-28 px-3 py-2 rounded-xl bg-black/5 border border-white/10 text-sm text-slate-800 placeholder-slate-500 focus:outline-none focus:border-secondary transition-all"
                      />
                      <input
                        type="text"
                        ...
                        placeholder="节点任务"
                        className="flex-1 px-3 py-2 rounded-xl bg-black/5 border border-white/10 text-sm text-slate-800 placeholder-slate-500 focus:outline-none focus:border-secondary transition-all"
                      />
                      <button
                        type="button"
                        onClick={() => removeMilestone(plan.id, index)}
                        className="w-9 h-9 rounded-xl bg-black/5 flex items-center justify-center text-slate-600 hover:text-danger hover:bg-danger/10 transition-all"
                        aria-label="删除节点"
                      >
          ...
          <div className="relative z-10 p-6 pt-4 border-t border-white/5 flex items-center justify-end gap-3">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-secondary to-accent text-text-primary text-sm font-medium hover:shadow-[0_0_20px_rgba(139,92,246,0.4)] transition-all"
            >
