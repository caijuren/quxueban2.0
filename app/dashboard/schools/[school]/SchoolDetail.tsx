    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center space-y-6">
        ...
        <div>
          ...
          <p className="text-slate-600">当前链接对应的目标学校不存在。</p>
        </div>
        <Link
          href="/dashboard/plan"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-black/5 text-slate-800 hover:bg-black/10 transition-all"
        >
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="space-y-6">
        <Link
          href="/dashboard/plan"
          className="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-text-primary transition-colors"
        >
          <ArrowLeft className="w-4 h-4" />
          返回路线方案
        </Link>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-5">
                {data.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-3 py-1 rounded-lg bg-black/5 text-sm text-slate-700 border border-white/10"
                  >
                    {tag}
                  </span>
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div variants={itemVariants} className="space-y-4">
          ...
          <div className="rounded-2xl glass p-6 border border-white/5">
            <p className="text-sm text-slate-600 mb-4">学校最看重的能力与素养</p>
            <div className="flex flex-wrap gap-2">
              {data.preferences.map((pref) => (
                <span
                  key={pref}
                  className="px-3 py-1.5 rounded-lg text-sm text-slate-800 border border-white/10"
                  style={{
          ...
          <div className="rounded-2xl glass p-6 border border-white/5 space-y-4">
            {data.interviewFormat.map((item) => (
              <div key={item} className="flex items-start gap-3">
                {item.includes('机考') ? (
                  <Monitor className="w-4 h-4 text-slate-600 mt-1 shrink-0" />
                ) : item.includes('小组') ? (
                  <Users className="w-4 h-4 text-slate-600 mt-1 shrink-0" />
                ) : (
                  <MessageSquare className="w-4 h-4 text-slate-600 mt-1 shrink-0" />
                )}
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">{group.sub}</p>
                </div>
              </div>
              <ul className="space-y-3">
                {group.tasks.map((task) => (
                  <li key={task} className="flex items-start gap-3 text-sm text-slate-700">
                    <span
        ...
        <div className="rounded-2xl glass p-6 border border-white/5">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.certificates.map((cert) => (
              <div
                key={cert}
                className="flex items-center gap-3 p-4 rounded-xl bg-black/[0.03] border border-white/5"
              >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-slate-800">继续规划升学路线</p>
            <p className="text-xs text-slate-600">返回路线方案页查看全部目标与进度</p>
          </div>
        </div>
        <Link
          href="/dashboard/plan"
          className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-primary to-secondary text-text-primary font-semibold hover:shadow-[0_0_30px_rgba(244,63,94,0.5)] transition-all duration-300"
        >
