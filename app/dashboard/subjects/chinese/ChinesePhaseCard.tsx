    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="rounded-xl bg-black/5 p-4 border border-white/5">
          <p className="text-xs text-slate-600 mb-1">当前主题</p>
          <p className="text-lg font-bold text-slate-800">{status.currentTopic}</p>
        </div>

        <div className="rounded-xl bg-black/5 p-4 border border-white/5">
          <p className="text-xs text-slate-600 mb-1">每日语文</p>
          <p className="text-2xl font-bold text-slate-800">{status.dailyChineseTime}</p>
          <p className="text-xs text-slate-600 mt-1">含读写背</p>
        </div>
        ...
        <div className="rounded-xl bg-black/5 p-4 border border-white/5">
          <p className="text-xs text-slate-600 mb-1">下一节点</p>
          <p className="text-lg font-bold text-slate-800">{status.nextEvent}</p>
          <p className="text-xs text-slate-600 mt-1">{status.nextEventDate}</p>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="flex items-start gap-3 rounded-xl bg-primary/5 border border-primary/20 p-4">
          ...
          <div>
            <p className="font-medium text-slate-800 mb-1">下一节点：{status.nextEvent}</p>
            <p className="text-sm text-slate-600">目标时间：{status.nextEventDate}</p>
          </div>
        </div>
