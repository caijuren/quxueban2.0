...
本版本是 v2.8 视觉升级系列的最终整合发布，覆盖移动端适配、全局回归测试与发布。

- 移动端优化
  - 首页卡片改为单列堆叠，减少 Glass 卡片数量避免小屏拥挤
  - 设置页左侧导航在移动端调整为顶部 Tab 布局
  - 成长报告学科分析卡片单列，图表横向滚动适配
  - AI 助手输入框固定在底部，消息气泡宽度适配
  - 底部操作区点击区域增大至 ≥ 44px
- 全局回归修复
  - 160+ 个文件涉及移动端样式修复与布局调整
  - 覆盖全站页面：首页、成长报告、AI 模块、设置中心、周计划、规划工具、学科页、工具箱、营销页等
  - 深色/浅色主题双验证
- 质量验证
...
本版本在 v2.8.3 图标字体升级基础上，统一数据展示组件与空/加载/错误态。

- 数据可视化组件
  - 新增 `MetricCard` 组件：大数字 + 趋势箭头 + 标签，支持玻璃/实色两种风格
  - 新增 `ProgressRing` 组件：品牌渐变进度圆环，支持尺寸变体与动画过渡
  - 新增 `TrendChart` 组件：统一 tooltip、轴线、网格样式，支持深色/浅色自动适配
  - 新增 `Heatmap` 组件：用于打卡热力图，统一颜色梯度
- 空状态升级
  - 重构 `EmptyState` 组件：插画 + 标题 + 描述 + 操作按钮，支持多种预设场景
- 加载态升级
  - 优化 `Skeleton` 组件：支持 pulse / shimmer 两种动画
- 错误态升级
  - 新增 `ErrorState` 组件：错误图标 + 信息 + 重试按钮，支持 403/404/500/网络错误预设
- 质量验证
  - `npm run type-check` 通过
  - `npm run lint` 通过
  - `npm run build` 通过
...
本版本在 v2.8.2 动效系统基础上，建立统一的图标管理与字体排版体系。

- 图标统一
  - 新增 `Icon` 组件（`components/ui/icon.tsx`）：统一从 Lucide 导入，支持 `size`、`variant`、动画状态
  - 全站搜索替换散落的 lucide 图标直接引用，统一走 Icon 组件
- 字体排版优化
  - 扩展 `tailwind.config.ts` 字体配置，增加数字等宽字体避免抖动
  - 中文字体栈优化：标题使用 `PingFang SC` + 备选 `HarmonyOS Sans SC`
- 质量验证
  - `npm run type-check` 通过
  - `npm run lint` 通过
  - `npm run build` 通过
