      ...
      {open && task && (
        ...
        >
          <motion.div
            ...
            className="w-full sm:w-full sm:max-w-lg max-h-[85vh] overflow-y-auto modal-scroll rounded-t-2xl sm:rounded-2xl bg-background border border-border-default shadow-2xl"
          >
            <div className="sticky top-0 z-10 flex items-center justify-between gap-3 px-5 py-4 border-b border-border-default bg-background/95 backdrop-blur">
              <div>
                <h2 className="text-lg font-bold font-display text-text-primary">
                  记录完成情况
                </h2>
                <p className="text-xs text-text-tertiary mt-0.5 truncate max-w-[260px] sm:max-w-[320px]">
                  {task.focus}
                </p>
              </div>
              <button
                onClick={onClose}
                className="p-2 rounded-xl hover:bg-surface-light text-text-tertiary transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
