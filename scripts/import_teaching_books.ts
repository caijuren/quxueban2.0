...
/**
 * 一次性教辅书目导入脚本
 * 数据源：prisma/seed-data/*.json（由 convert_teaching_books.py 生成）
 * 执行方式：pnpm tsx scripts/import_teaching_books.ts
 */
...
async function main() {
  try {
    ...
    const contentTypeMap = await upsertContentTypes();
    console.log(`内容类型导入完成：${contentTypeMap.size} 条`);

    console.log('开始导入书目...');
    const stats = await upsertBooks(publisherMap, contentTypeMap);
